    ...
    executeWithRange(modeHandler, range) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            for (let currentLine = startLine.line; currentLine <= endLine.line && currentLine < textEditor_1.TextEditor.getLineCount(); currentLine++) {
                let originalContent = textEditor_1.TextEditor.readLineAt(currentLine);
                let content = originalContent.replace(regex, this._arguments.replace);
                if (originalContent !== content) {
                    yield textEditor_1.TextEditor.replace(new vscode.Range(currentLine, 0, currentLine, originalContent.length), content);
                }
            }
        });
    }
